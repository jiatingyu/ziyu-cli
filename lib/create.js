...
module.exports = async function (proName) {
  ...
  let { data } = await axios.get(
    "https://api.github.com/users/jiatingyu/repos"
  );

  // 将上述的数据处理一下，返回一个数组，这样便于我们下面做交互选择
  let repoList = data.map((item) => item.name);
  let cloneMap = {};
  data.forEach((item) => {
  ...
  let { repoName } = await inquirer.prompt(promptList);
      ...
      if (!err) {
        ...
        shelljs.exec("npm i ", function () {
          let res = fs.readFileSync(
            path.join(process.cwd(), proName, "README.md"),
            "utf-8"
          );
